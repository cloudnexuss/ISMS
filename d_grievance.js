...
const searchGrievance = () => {
    ...
    if (status.toLowerCase() === "all" || status.trim() === "") {
        url = `https://ismsb.onrender.com/grievances/by-email/full?email=${encodeURIComponent(email)}`;
    } else {
        url = `https://ismsb.onrender.com/grievances/by-email/status?email=${encodeURIComponent(email)}&status=${encodeURIComponent(status)}`;
    }
...
const showGrievanceDetails = async (grievance) => {
    ...
    try {
        const response = await fetch("https://ismsb.onrender.com/officials/notations");
...
const getNotationName = async (email) => {
    try {
        const response = await fetch(`https://ismsb.onrender.com/officials/notation?email=${encodeURIComponent(email.trim())}`);
...
document.getElementById("forwardChainConfirmBtn").addEventListener("click", async () => {
    ...
    try {
        ...
        const url = new URL("https://ismsb.onrender.com/grievances/forward");
...
document.querySelector(".resolve-btn").addEventListener("click", async () => {
    ...
    try {
        ...
        const url = new URL("https://ismsb.onrender.com/grievances/resolve");
        url.searchParams.append("grievanceId", grievanceId);
